...
class Iphone {
  constructor(img, Nombre, Modelo, Precio) {
    this.Nombre = Nombre;
    this.Modelo = Modelo;
    this.Precio = Precio;
    this.img = img;
  ...
  mostrar() {
    return `${this.img}Nombre: ${this.Nombre} <br>Modelo: ${this.Modelo} <br> <p style = "color: green;"> $${this.Precio}</p>`;
  }
}
...
comprar.addEventListener("click", () => {
  if (Compras.length != 0) {
    let confirmacion = confirm("Confirmar compra?");
    if (confirmacion) {
